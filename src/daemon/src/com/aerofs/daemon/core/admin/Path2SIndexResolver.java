...
public class Path2SIndexResolver
{
    ...
    @Inject
    public Path2SIndexResolver(IMapSID2SIndex sid2sidx, DirectoryService ds,
    ...
    SIndex getSIndex_(Path path) throws SQLException, ExExpelled, ExNotShared, ExNotFound
    {
        ...
        if (_urdb.getUnlinkedRoot(path.sid()) != null) {
            // Unlinked store.
            sidx = _sid2sidx.getLocalOrAbsentNullable_(path.sid());
        } else {
            ...
            if (oa.isAnchor()) {
                sidx = _sid2sidx.getLocalOrAbsentNullable_(SID.anchorOID2storeSID(oa.soid().oid()));
            } else {
