...
public class DPUTMigrateStorageConfig implements IDaemonPostUpdateTask
{
    ...
    public void run() throws Exception {
        // Legacy value ?
        String legacyPassword = _db.getNullable(CfgDatabase.Key.S3_LEGACY_ENCRYPTION_PASSWORD);

        if (legacyPassword != null) {
            // Abort if already existing
            if (_db.getNullable(CfgDatabase.Key.STORAGE_ENCRYPTION_PASSWORD) != null) {
                l.debug("There is already a STORAGE_ENCRYPTION_PASSWORD value in the db along the legacy one. Aborting.");
                return;
            }

            _db.set(CfgDatabase.Key.STORAGE_ENCRYPTION_PASSWORD, legacyPassword);
        }
    }
}
