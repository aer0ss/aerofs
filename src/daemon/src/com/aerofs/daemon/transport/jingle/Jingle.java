...
import com.aerofs.daemon.transport.jingle.routing.IConnectionServiceListener;
import com.aerofs.daemon.transport.jingle.routing.ISignalledConnectionService;
...
import com.aerofs.daemon.transport.jingle.routing.ConnectionServiceWrapper;
...
/**
 * Acts as a controller (or wrapper) over a number of {@link com.aerofs.daemon.transport.jingle.routing.IConnectionService} implementations
 ...
 * </ol>
 */
public class Jingle implements ITransportImpl, IConnectionServiceListener, IUnicast,
        IXMPPConnectionServiceListener
{
    public Jingle(
            DID localdid,
            byte[] scrypted,
            ...
            RockLog rocklog)
    {
        ...
        _xsc = new XMPPConnectionService(localdid, id(), scrypted, this, rocklog);
        _mc = new Multicast(this, maxcastFilterReceiver, _xsc, sink, localdid, id());
        _pm.addPulseDeletionWatcher(new GenericPulseDeletionWatcher(this, _sink));
        _spf = new ConnectionServiceWrapper(_sched, new JingleWrapper(localdid, getXmppPassword(), absRTRoot, id, rank, this, transportStats));
    ...
    {
        return new HdPulse<EOTpStartPulse>(new StartPulse<Jingle>(this, _xpm));
    }
    ...
    public final void onDeviceConnected(final DID d, final IConnectionService cs)
    {
        ...
        enqueueIntoXmpp(new AbstractEBSelfHandling()
        {
            ...
            public void handle_()
            {
                ...
                TPUtil.sessionEnded(new Endpoint(Jingle.this, d), _sink, _sm, true, true);
    ...
    {
        try {
            Endpoint ep = new Endpoint(Jingle.this, did);
    ...
    public void xmppServerDisconnected()
    {
        ...
        AbstractEBSelfHandling serverDisconnectedEvent = new AbstractEBSelfHandling()
        {
            ...
            {
                try {
                    _sink.enqueueBlocking(new EIPresence(Jingle.this, false, ImmutableMap.<DID, Collection<SID>>of()), Prio.LO);
     ...
     * cannot be processed by {@link com.aerofs.daemon.transport.jingle.routing.ConnectionServiceWrapper} due to resource constraints
    ...
    private final PresenceStore _xpm = new PresenceStore();
    ...
    protected static final Logger l = Loggers.getLogger(Jingle.class);
}
