...
public class StateMachine<T extends IStateContext>
{
     ...
     * @return either {@code HALT} (state machine terminated abnormally for this context
     * and has to be shut down) or {@code PARK} (state machine needs to wait for an external
    ...
    public CoreEvent run_(T ctx) throws ExInvalidTransition
    {
        ...
        try {
            while (true) {
                ...
                checkState(ev != HALT, "nextev_ return bad ev:" + ev);
                if (ev == PARK) break;
     ...
     * @return true if the event is one of {@code PARK} or {@code HALT}
     */
    ...
    private StateMachineEvent nextev_(T ctx)
    {
        ...
        while (ev == null) {
            ...
            checkState(!coreev_(ev), "core ev in ctx eq ev:" + ev); // no explicit PARK/HALT
            checkState(!defer.contains(ev.type()), "dequeue returned deferred ev:" + ev);
        }
     ...
     * the {@link IStateContext} object is in, and the {@link StateMachineEvent} that occurred
     *
        ...
        throws ExInvalidTransition
    {
        ...
        checkState(transmap != null && !transmap.isEmpty(), "no transitions:" + transmap + " for state:" + prev);
        ...
        ctx.logger_().trace("{}: T: ({}) {}->{}", ctx, ev, prev, next);
    }
    ...
    private StateMachineEvent process_(IState<T> state, StateMachineEvent inev, T ctx)
    {
        ...
        ctx.logger_().trace("{}: R: ({}) {}", ctx, retev, ctx.curr_());
