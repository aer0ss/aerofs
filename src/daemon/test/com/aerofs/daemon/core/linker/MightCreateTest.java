...
import com.aerofs.daemon.core.linker.MightCreateOperations.Operation;
...
public class MightCreateTest extends AbstractMightCreateTest
{
    ...
    {
        PathCombo pc = new PathCombo(cfgAbsRootAnchor, Path.fromString("foo"));
    ...
    {
        SOID soid = ds.resolveNullable_(Path.fromString("f1"));
    ...
    {
        SOID soid = ds.resolveNullable_(Path.fromString("d0"));
    ...
    {
        SOID soid = ds.resolveNullable_(Path.fromString("d-expelled"));
        generateDirFnt(soid);
        FIDAndType fnt = generateDirFnt();
    ...
    {
        SOID target = ds.resolveNullable_(Path.fromString("f1"));
        generateFileFnt(target);

        SOID source = ds.resolveNullable_(Path.fromString("d0"));
    ...
    {
        SOID target = ds.resolveNullable_(Path.fromString("d0"));
        generateDirFnt(target);

        SOID source = ds.resolveNullable_(Path.fromString("f1"));
    ...
    {
        SOID target = ds.resolveNullable_(Path.fromString("d-expelled"));
        generateDirFnt(target);
