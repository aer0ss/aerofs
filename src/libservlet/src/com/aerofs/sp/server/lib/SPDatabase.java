...
public class SPDatabase extends AbstractSQLDatabase
{
    ...
    public void insertPasswordResetToken(UserID userId, String token)
        throws SQLException
    {
        try (PreparedStatement ps = prepareStatement("insert into " +
    ...
    {
        try (PreparedStatement ps = prepareStatement("select " + C_PASS_USER +
    ...
    public void deletePasswordResetToken(String token)
        throws SQLException
    {
        try (PreparedStatement psDPRT = prepareStatement("delete from " + T_PASSWORD_RESET +
                " where " + C_PASS_TOKEN + " = ?")) {
    ...
    {
        try (PreparedStatement psTASUC = getConnection().prepareCall(
    ...
    public Set<UserID> getSharedUsersSet(UserID userId) throws SQLException
    {
        ...
        try (PreparedStatement psGSUS = prepareStatement(
            ...
            try (ResultSet rs = psGSUS.executeQuery()) {
                while (rs.next()) {
    ...
    public @Nonnull UserID getSignUpCode(String code)
        throws SQLException, ExNotFound
    {
        try (PreparedStatement ps = prepareStatement(DBUtil.selectWhere(T_SIGNUP_CODE, C_SIGNUP_CODE_CODE + "=?",
                C_SIGNUP_CODE_TO))) {
