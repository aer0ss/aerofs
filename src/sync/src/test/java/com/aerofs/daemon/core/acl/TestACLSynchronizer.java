...
public class TestACLSynchronizer extends AbstractTest
{
    ...
    IACLDatabase adb = new ACLDatabase(idbcw);
    ...
    public void setUp() throws Exception
    {
        ...
        idbcw.init_();
        try (Statement s = idbcw.getConnection().createStatement()) {
            new SyncSchema().create_(s, idbcw);
        }
    ...
    public void shouldJoinFolderWhenAccessGained() throws Exception
    {
        ...
        verify(storeJoiner).joinStore_(eq(sidx), eq(sid1), sf(SHARED_FOLDER_NAME, external),
                eq(t));
    }

    public static StoreInfo sf(String name, boolean external)
    {
        return argThat(new BaseMatcher<StoreInfo>() {
            @Override
            public boolean matches(Object item) {
                StoreInfo info = (StoreInfo)item;
                return info._name.equals(name) && info._external == external;
            }
    ...
    public void shouldNotJoinFolderWhenAccessGainedForUserOutOfShard() throws Exception
    {
        ...
        verify(storeJoiner, never()).joinStore_(eq(sidx), eq(sid1), sf(SHARED_FOLDER_NAME, external),
                eq(t));
    }
    ...
    public void shouldNotJoinOrLeaveFolderWhenAccessUnchanged() throws Exception
    {
        ...
        verify(spClient).getACL(anyLong());
        verify(storeJoiner).onMembershipChange_(eq(sidx), any(StoreInfo.class), eq(t));
