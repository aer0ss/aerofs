...
public final class ObjectStore {
    ...
    private Updated insertChild(DAO dao, DID device, UniqueID parentOid, UniqueID childOid, @Nullable ObjectType childObjectType, byte[] childName) throws ParentConflictException, NotFoundException, NameConflictException
    {
    ...
    private Updated removeChild(DAO dao, DID device, UniqueID parentOid, UniqueID childOid) throws NotFoundException {
    ...
    private Updated migrateObject(DAO dao, DID device, UniqueID parentOid, UniqueID newOid, UniqueID migrantOid, byte[] name)
    {
        ...
        if (migrant.objectType == ObjectType.FILE && migrant.version > Constants.INITIAL_OBJECT_VERSION) {
            ...
            update = makeContent(dao, device, newOid, 0L, currentContent.hash, currentContent.size, currentContent.mtime);
        }
        return update;
    }
