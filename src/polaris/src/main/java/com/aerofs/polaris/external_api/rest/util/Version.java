package com.aerofs.polaris.external_api.rest.util;
...
public class Version implements Comparable<Version>
{
    ...
    Version fromRequestPath(String path)
    {
        if (!path.startsWith("/v")) return null;

        int vidx = path.indexOf('v');
        int idx = path.indexOf('/', vidx + 1);
        if (idx == -1) return null;
        return Version.fromStringNullable(path.substring(vidx + 1, idx));
    }
