...
public class TestConversionResource {
    ...
    public void shouldIgnoreUpdatesIfVersionDoesNotDominate() throws Exception
    {
        ...
        // not a dominating version
        version.put(device2, 1L);
        batch = new TransformBatch(Lists.newArrayList(
                new TransformBatchOperation(store, insertChild(child, ObjectType.FILE, "new_name_for_file", null, version, null)),
                new TransformBatchOperation(child, new UpdateContent(0L, hash, 200, 2048, version))));
        submitBatchSuccessfully(store, batch);
    ...
    public void shouldRenameOnNameConflictsWithDominatingVersion() throws Exception
    {
        ...
        TransformBatch batch = new TransformBatch(Lists.newArrayList(
                ...
                // same name under same parent, renames the first objecet
    ...
    @Test
    public void shouldDropOnNameConflictsWithNonDominatingVersion() throws Exception
    {
        SID store = SID.generate();
        OID child = OID.generate();
        ...
        TransformBatch batch = new TransformBatch(Lists.newArrayList(
                new TransformBatchOperation(store, insertChild(child, ObjectType.FOLDER, "object", null, version, null)),
                new TransformBatchOperation(store, insertChild(OID.generate(), ObjectType.FOLDER, "object", null, conflictVersion, null)),
                new TransformBatchOperation(store, insertChild(OID.generate(), ObjectType.FOLDER, "object", null, nonDomination, null))));
    ...
    public void previouslySubmittedObjectsRemovedUponFindingAlias() throws Exception
    {
        ...
        // TODO (RD) add some children under the two objects to be merged
        TransformBatch batch = new TransformBatch(Lists.newArrayList(
    ...
    @Test
    public void shouldResolveMultipleAliasses() throws Exception
    {
        SID store = SID.generate();
