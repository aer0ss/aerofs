...
hpc_server = Table('hpc_server', post_meta,
    ...
    Column('docker_url', String(length=256)),
    Column('public_ip', String(length=15)),
    Column('upgrade_status', Boolean, server_default=text("False"))
)
...
def upgrade(migrate_engine):
    ...
    post_meta.tables['hpc_server'].columns['upgrade_status'].create()
...
def downgrade(migrate_engine):
    ...
    pre_meta.bind = migrate_engine
    post_meta.bind = migrate_engine
    post_meta.tables['hpc_server'].columns['upgrade_status'].drop()
