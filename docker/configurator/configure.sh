...
if [ $# != 4 ]; then
    (set +x
        ...
        echo "       <hostname> <ip> hostname & ip of the appliance under test. It must be consistent with the CNAME"
        echo "                      of the browser cert defined in appliance.yml otherwise tests would fail."
...
SCREEN_SHOTS="${THIS_DIR}/../../out.shell/configurator-screenshots"
mkdir -p "${SCREEN_SHOTS}"
rm -rf "${SCREEN_SHOTS}"/*

docker build -t aerofs/configurator "${THIS_DIR}"
