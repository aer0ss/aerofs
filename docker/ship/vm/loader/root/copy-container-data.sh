...
docker run --rm -v ${HOST_FOLDER}:${BIND_FOLDER} --volumes-from ${SOURCE} --entrypoint bash ${LOADER_IMAGE} \
...
docker run --rm -v ${HOST_FOLDER}:${BIND_FOLDER} --volumes-from ${TARGET} --entrypoint bash ${LOADER_IMAGE} \
