...
if [ $# != 3 ]; then
    (set +x
        echo "Usage: $0 <hostname> <ip> <screenshots_output_dir>"
        echo "       <hostname> hostname of the appliance under test. It must be consistent with the CNAME of the browser"
        echo "                  cert defined in appliance-setup.yml otherwise tests would fail."
...
(set +e
    docker run --rm \
        ...
        \
        config-appliance \
        /run.sh "${HOST}" /screenshots /test.license /appliance.yml
    EXIT_CODE=$?
